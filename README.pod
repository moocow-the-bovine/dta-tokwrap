...
Script to splice C<E<lt>wE<gt>> elements encoded from a stand-off (.w.xml) XML file
into a compatible (but not necessarily the I<original>) "base-format" (.char.xml) file.
Quite slow, and a tad too generous with partial word segments, due to
...
Script to splice C<E<lt>sE<gt>> elements encoded from a stand-off (.s.xml) XML file
into a compatible word-spliced (.cw.xml) file as output by the L</dtatw-add-w.perl> script.
Quite slow, and a tad too generous with partial sentence segments, due to
strict adjacency and boundary criteria.
