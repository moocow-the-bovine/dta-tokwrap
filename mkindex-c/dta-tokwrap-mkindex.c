...
void put_record_start(TokWrapData *data, const XML_Char *name, const XML_Char **attrs)
{
  ...
  put_record_raw(data->f_sx, START_ID, xoffset,xlen, data->c_toffset,-1, name);
  for (ai=0; attrs[ai]; ai++) {
    put_record_raw(data->f_sx, ATTR_ID, xoffset,xlen, data->c_toffset,-1, attrs[ai]);
  }
}
...
{
  put_record_raw(data->f_sx, END_ID, XML_GetCurrentByteIndex(data->xp),XML_GetCurrentByteCount(data->xp), data->c_toffset,-1, name);
