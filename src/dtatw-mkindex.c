...
void print_indexfile_header(FILE *f, int argc, char **argv)
{
  ...
  if (want_outfile_comments) {
    int i;
    fprintf(f, "%%%% <c>-element index generated by %s\n", prog);
  ...
  if (want_outfile_colnames) {
    fprintf(f, "%s$ID$\t$XML_OFFSET$\t$XML_LENGTH$\t$TX_OFFSET$\t$TX_LEN$\t$PB_FACS$\t$TEXT$\n", (want_outfile_comments ? "%% " : ""));
  }
}
...
//--------------------------------------------------------------
void put_record_raw(FILE *f, const char *id, ByteOffset xoffset, int xlen, ByteOffset toffset, int tlen, int pb, const char *txt)
{
  if (!f) return;
  fprintf(f, "%s\t%lu\t%d\t%lu\t%d\t%d\t%s\n",
	  id,
	  xoffset,xlen,
...
void cb_start(TokWrapData *data, const XML_Char *name, const XML_Char **attrs)
{
  if (data->text_depth) {
	...
	exit(3);
      ...
      for (; *attrs; attrs += 2) {
	if (strcmp(*attrs,"id")==0 || strcmp(*attrs,"xml:id")==0) {
	  id = attrs[1];
      ...
      const XML_Char *facs = get_attr("facs",attrs);
      if (facs != NULL) {
	for ( ; facs && (*facs=='#' || *facs=='f' || *facs=='0'); facs++) ;
	data->pb_facs = strtol(facs,NULL,0);
	//-- don't just return here: keep <pb> elements in the *.sx file
	//data->total_depth++;
	//return;
      }
      put_record_pb(data);
